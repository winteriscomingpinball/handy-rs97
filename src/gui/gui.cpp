...
void print_string(const char *s, u16 fg_color, u16 bg_color, int x, int y, int size)
{
	...
	int i, j = strlen(s);
	if (size>1){
		sizemod=USED_FONT_WIDTH*size; //font width X size
	}else{
	    sizemod=USED_FONT_WIDTH+1; //font width + 1
	}
...
void ShowMenu(MENU *menu)
{
	...
	*(unsigned long *)&buf[16] = 0;
	...
	if(runRomBrowser){
		...
		if(menu->itemCur == i && strlen(gui_RomBrowserItems[menu->itemCur].itemName)>MAX_CHARACTERS_ACROSS){
			scrollval=true;
